...
## نظرة عامة
نظام ويب عربي (RTL) متكامل لإدارة عروض الأسعار وعروض الأتعاب، مبني بـ Hono + Supabase + Tailwind CSS.
...
### الميزات المتقدمة
- [x] تصدير PDF مع html2canvas + jsPDF (محلي - بدون CDN)
...
### قالب عرض الأتعاب الاحترافي (جديد)
- [x] **قالب PDF احترافي عربي** مطابق لنموذج عرض الأتعاب
- [x] **إعدادات قابلة للتخصيص بالكامل** عبر صفحة `/settings`:
  - بيانات الشركة (اسم عربي + إنجليزي، عنوان، هاتف، بريد، موقع، سجل تجاري، رقم ضريبي)
  - شعار الشركة (رابط مباشر)
  - ألوان القالب (رئيسي + ثانوي)
  - إعدادات الضريبة (إظهار/إخفاء + النسبة)
  - العملة (عربي + إنجليزي)
  - شروط الدفع (قابلة للإضافة والحذف)
  - الشروط والأحكام (قابلة للإضافة والحذف)
  - منطقة التوقيع (الطرف الأول والطرف الثاني + الختم)
  - نص التذييل
- [x] **معاينة مباشرة للقالب** قبل التصدير
- [x] **jsPDF + html2canvas محلي** (لا يعتمد على CDN خارجي)
- [x] **تحميل كسول (lazy loading)** مع إعادة محاولة تلقائية
- [x] **دعم الصفحات المتعددة** للعروض الطويلة
...
- **PDF**: jsPDF 2.5.2 + html2canvas 1.4.1 (محلي / static)
...
- `/settings` - إعدادات قالب عرض الأتعاب (جديد)
...
│   ├── app.js             # Frontend SPA
│   ├── style.css          # أنماط
│   ├── jspdf.umd.min.js   # jsPDF 2.5.2 (محلي)
│   └── html2canvas.min.js # html2canvas 1.4.1 (محلي)
├── supabase/
...
## دليل استخدام قالب عرض الأتعاب

### تخصيص القالب
1. اذهب إلى `/settings` من القائمة الرئيسية
2. أدخل بيانات شركتك (اسم، عنوان، هاتف، بريد، إلخ)
3. اختر ألوان القالب المناسبة لهوية شركتك
4. عدّل شروط الدفع والشروط والأحكام حسب حاجتك
5. اضغط "حفظ الإعدادات"
6. استخدم "معاينة القالب" للتحقق من المظهر

### تصدير عرض أتعاب كـ PDF
1. أنشئ عرض سعر جديد من `/quotes/new`
2. أضف البنود والتفاصيل
3. من صفحة تفاصيل العرض، اضغط زر "PDF"
4. سيتم تحميل الملف تلقائياً وحفظه في السحابة

### أقسام قالب PDF
- شريط ألوان علوي
- رأس المستند (بيانات الشركة + عنوان "عرض أتعاب" + رقم العرض)
- بيانات العميل (مقدم إلى)
- تفاصيل العرض (تاريخ، صلاحية، سجل تجاري، رقم ضريبي)
- موضوع العرض
- جدول بنود الأتعاب (م، البند، الكمية، سعر الوحدة، المبلغ)
- ملخص مالي (مجموع فرعي + ضريبة + إجمالي)
- ملاحظات
- شروط الدفع
- الشروط والأحكام
- منطقة التوقيع (الطرف الأول + الطرف الثاني + الختم)
- تذييل الصفحة
